...
// 出力結果をCSVに書き出す
void CAlconDatabase::WriteOutputs( map< int, vector<string> > outs, string fn, string lv ){
  ...
  for( vector<int>::iterator it=m_ids.begin(); it!=m_ids.end(); it++ ){
    // write ID
    ofs << *it;
    vector<string> guess = outs[ *it ];
    //cout << stripVectorStrings( guess ) << endl;

    // write guesses
    cout << guess.size() << endl;
    for( vector<string>::iterator it2=guess.begin(); it2!=guess.end(); it2++ ){
      ofs << "," << *it2;
    }
    ofs << endl;
  }
